...
metadata:
  ...
  namespace: sandbox
rules:
- apiGroups: ["anthos-cert-manager.io"]
  resources: ["signers"]
  verbs:     ["reference"]
  resourceNames:
...
annotation `experimental.anthos-cert-manager.io/request-duration` who's value is a [Go
...
issuers](https://anthos-cert-manager.io/docs/configuration/external/) are built for the
...
for cert-manager signers will be prefixed with `anthos-cert-manager.io` to prevent
conflicts with other external signer projects.

```yaml
  # Namespaced issuer reference
  # e.g. `issuers.anthos-cert-manager.io/my-namespace.my-issuer
  signerName: issuers.anthos-cert-manager.io/<namespace>.<issuer-name>

  # Cluster scoped issuer reference
  # e.g. `clusterissuers.anthos-cert-manager.io/my-issuer
  signerName: clusterissuers.anthos-cert-manager.io/<clusterissuer-name>
```
...
- SelfSigned: The `experimental.anthos-cert-manager.io/private-key-secret-name`
...
- Venafi:
  - The `venafi.experimental.anthos-cert-manager.io/custom-fields` annotation is set by
      the user to request a Venafi certificate with custom fields.
  - The `venafi.experimental.anthos-cert-manager.io/pickup-id` annotation is used by
...
- IsCA: The `experimental.anthos-cert-manager.io/request-is-ca` annotation is used to
    request a certificate with the X.509 `IsCA` flag set.

- Duration: The `experimental.anthos-cert-manager.io/request-duration` annotation is used to
...
1. Create signer implementations for each anthos-cert-manager.io issuer types. These
